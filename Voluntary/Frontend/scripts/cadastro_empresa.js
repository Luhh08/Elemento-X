...
const form = document.getElementById("form2"); // formulário final (empresa + representante)
...
form.addEventListener("submit", async (e) => {
  ...
  // Validação mínima representante
  ...
  try {
    // Envia dados da empresa para /empresa
    ...
    if (!responseEmpresa.ok) {
      alert("Erro ao criar empresa: " + (resultadoEmpresa.error || "Erro desconhecido"));
      return;
    }

    // Envia dados do representante legal para /representante
    const responseRep = await fetch("http://localhost:3000/representante", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(dadosRepresentante)
    });

    const resultadoRep = await responseRep.json();
    if (!responseRep.ok) {
      alert("Erro ao criar representante: " + (resultadoRep.error || "Erro desconhecido"));
      return;
    }

    alert("Cadastro concluído com sucesso!");
    form.reset(); // limpa o formulário
    window.location.href = "login_empresa.html"; // redireciona para login
